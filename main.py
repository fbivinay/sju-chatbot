...
def search_supabase(question: str) -> str:
    """Optionally get extra info from Supabase crawled data"""
    if not sb:
        return ""
    try:
        stop = {"what","when","where","how","is","are","the","a","an","at","in","of",
                "for","to","do","does","tell","me","about","i","can","will","which",
                "who","sju","college","university","please","give","yes","more"}
        words = [w.lower().strip("?.,!") for w in question.split()
                 if w.lower() not in stop and len(w) > 2]
        if not words:
            return ""
        term = " ".join(words[:4])
        r = sb.table("sju_knowledge").select("content").ilike("content", f"%{term}%").limit(3).execute()
        if r.data:
            chunks = [row["content"][:500] for row in r.data]
            return "=== EXTRA INFO FROM SJU WEBSITE ===\n" + "\n\n".join(chunks)
    except Exception:
        pass
    return ""
